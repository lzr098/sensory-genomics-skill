...
class ImpactEngine:
    ...
    def _determine_level(
        ...
        # 纯合 LoF → 完全丧失（OR 基因）
        if variant.is_homozygous and ("frameshift" in consequence or "stop_gained" in consequence):
            if gene_sets.get_subsystem(gene) == "olfaction" and gene.startswith("OR"):
                return "完全丧失"
            if gene in ("GJB2", "GJB6", "OTOF", "SLC26A4", "MYO7A"):
                return "显著影响"
            if gene in ("OPN1LW", "OPN1MW"):
                return "显著影响"
        ...
        # 杂合 LoF 但显性遗传 → 显著影响
        if variant.is_heterozygous and ("frameshift" in consequence or "stop_gained" in consequence):
            if inheritance == "显性":
                return "显著影响"

        # 隐性纯合错义
        if variant.is_homozygous and "missense" in consequence:
            if gene in ("GJB2", "GJB6", "OTOF", "SLC26A4"):
                return "部分影响"
        ...
        # 杂合错义
        if variant.is_heterozygous and "missense" in consequence:
            if gene in ("OPN1LW", "OPN1MW", "OPN1SW"):
                return "可能轻微影响"
            if gene in ("TRPV1", "TRPM8"):
                return "可能轻微影响"
            if gene.startswith("OR"):
                return "无影响"  # OR 杂合不展示，但评估为无影响
            return "可能轻微影响"

        # 同义 / UTR
        if "synonymous" in consequence or "UTR" in consequence:
            return "无影响"
